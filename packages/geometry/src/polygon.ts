...
export function isCcw(polygon: readonly Vec2[]): boolean {
...
export function polygonAreaM2(polygon: readonly Vec2[]): number {
  return Math.abs(signedArea(polygon));
}

export function polygonPerimeterM(polygon: readonly Vec2[]): number {
  if (polygon.length < 2) {
    return 0;
  }
  let sum = 0;
